...
# options:
#   - [-h] for help
#   - [-path PATH] for indicate the path of library: default it use: $HOME/miniconda3/share/OpenCV/haarcascades/'
...
# Function which detect faces in 'img' image with 'face_cascade'
def detectFaces(img):
...
def blurringFace(descriptor, img):
    ...
    for (x, y, w, h) in faces:
        ...
        if len(matches) >= minMatches :
            doBlurring(x,y,w,h,faceImg,sigma)


# One Face Mode -> Face selected with ROI
def oneFaceMode(cap):
    global lButtonUp, drawing
